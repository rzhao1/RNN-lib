...
from models.word_seq2seq import Past2Seq
...
def main():
    ...
    with tf.Session() as sess:
        ...
        with tf.variable_scope("model", reuse=None, initializer=initializer):
            model = Past2Seq(sess, config, len(train_feed.vocab), train_feed.EOS_ID,
                                        log_dir=None if FLAGS.forward else log_dir, forward=False)

        with tf.variable_scope("model", reuse=True, initializer=initializer):
            valid_model = Past2Seq(sess, valid_config, len(train_feed.vocab), train_feed.EOS_ID, None, forward=False)

        # get a random batch and do forward decoding. Print the most likely response
        with tf.variable_scope("model", reuse=True, initializer=initializer):
            test_model = Past2Seq(sess, test_config, len(train_feed.vocab), train_feed.EOS_ID, None, forward=True)
